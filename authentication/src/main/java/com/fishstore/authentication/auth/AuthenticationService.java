package com.fishstore.authentication.auth;
...
public class AuthenticationService {
   /* private final UserRepository repository;
    ...
    public void revokeAllValidTokensForUser(User user) {
        try {
            // Assuming your repository supports a custom bulk update method
            //int updatedCount = tokenRepository.revokeAllValidTokensByUserId(user.getId());
            // Optional: Log the result
            //log.info("Revoked {} tokens for user with ID {}", updatedCount, user.getId());
        } catch (Exception e) {
            // Log the exception
            //log.error("Error revoking tokens for user with ID {}: {}", user.getId(), e.getMessage());
